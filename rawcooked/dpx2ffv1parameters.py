...
parser.add_argument('--textlist', action='store', dest='textlist_path', type=str, help='Pull list of files to process from a text file')


parser.add_argument('--dpx2ffv1', required='--framerate' in sys.argv, action='store_true', help='For each folder in input, creates an equivalent folder in output and converts the DPX sequence located in input/folder/pm to FFV1 using RAWcooked')
...
parser.add_argument('--checklogs', required=False, action='store_true', help='For each folder in input, check for a verification_log.txt file in input/folder/pm and report')


parser.add_argument('--lessparanoid', action='store_true', required=False, help='Include in dpx2ffv1 conversion to decode FFV1 file back to DPX and check md5s, but not compare ac/pm runtimes')
